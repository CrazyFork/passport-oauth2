...
OAuth2Strategy.prototype.authenticate = function(req, options) {
  ...
  //bm: 这步是refresh token回调的逻辑，资源网站带着refresh token请求返回了
  // 具体可以看 passport-github 的 example 目录的使用
  if (req.query && req.query.code) {
    ...
    try {
      ...
      if (arity == 4) {
        this._stateStore.verify(req, state, meta, loaded); // verify state 参数是否一致
      } else { // arity == 3
    ...
    //bm: 这步是请求 refresh token, 跳转到授权页面
